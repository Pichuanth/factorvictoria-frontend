...
export function buildParlay(candidatesByFixture, target, opts = {}) {
  ...
  const poolMatches = (() => {
    ...
    const optsTotal = safeNum(opts?.totalMatches, 0);
    const s = new Set();
    ...
    return Math.max(keyCount, idCount, clamp(heuristic, 0, 60), optsTotal);
  })();
...
minLegs = clamp(minLegs, 2, 15);
  ...
  const hardCap = 2.50; // hard cap for any single leg in parlays
  const capLegOddBase = (() => {
  ...
  function greedyBuild({ relax = false } = {}) {
    ...
    // Seed with a 'safe filler' (AH/OU) when we have enough matches and target is mid/high,
    // to avoid ugly BTTS chains and to help reach higher leg counts with lower odds.
    if (poolMatches >= 10 && t >= 20) {
      for (const c of scored) {
        ...
        if (odd <= 1.01 || odd > maxLegOddEff) continue;
      if (isBttsNo(c) && bttsNoCount >= 1) continue;
        if (isBttsNo(c)) continue;
        if (!isSafeFiller(c)) continue;
