...
export default function LandingPage() {
  ...
  return (
    <div>
      {/* Header brand */}
      <header className="fv-bg text-white">
      ...
      <section className="fv-bg text-white">
        <div className="max-w-6xl mx-auto px-4 py-12 text-center">
          ...
          <div className="mt-6">
            <a href="#planes" className="fv-btn-primary inline-block">Ver planes</a>
          </div>
        </div>
      </section>
      ...
      <main className="bg-gray-50">
        <div id="planes" className="max-w-6xl mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {plans.map((p)=>(
              <div key={p.key} className={`relative rounded-2xl border bg-white p-6 flex flex-col shadow-sm ${p.highlight?"border-[var(--fv-gold)] shadow-xl":"border-gray-200"}`}>
                {p.highlight && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 text-xs font-semibold rounded-full" style={{background:"linear-gradient(90deg,#D4AF37,#F0D98A)", color:"#1c2230"}}>
                    Más popular
                  </div>
                )}
          ...
          <div className="bg-white rounded-2xl shadow-md p-8 max-w-3xl mx-auto">
            ...
            <p className="text-center text-gray-600 mb-6">Ingresa un monto a apostar y descubre cuánto podrías ganar.</p>
            <div className="flex justify-center mb-6">
              <input type="number" className="w-48 px-4 py-2 border rounded-lg text-center" value={amount} min="1000" onChange={(e)=>setAmount(Math.max(0, Number(e.target.value||0)))} />
            </div>
            ...
            <p className="mt-6 text-center text-xs text-gray-500">*Las cuotas dependen del medio/emisor de la tarjeta.</p>
          </div>
        </div>
      </main>
    </div>
